#!/bin/bash
EFIPARTITION="$(blkid -s UUID -o value /dev/sda1)"
CRYPTBOOT="$(blkid -s UUID -o value /dev/sda2)"
CRYPTLVM="$(blkid -s UUID -o value /dev/sda5)"
CRYPTKEY="$(blkid -s PARTUUID -o value /dev/sdb2)"
